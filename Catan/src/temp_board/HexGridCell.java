...
{
    private static final int[] NEIGHBORS_DJ = { 0, 1, 1, 0, -1, -1 };
    private static final int[][] NEIGHBORS_DI = { 
    ...
    public HexGridCell(int radius) {
        ...
        int cdx[] = {0,HEIGHT/2,HEIGHT,HEIGHT,HEIGHT/2,0}; // coordinate values starting at the upper left corner clockwise order
        CORNERS_DX = cdx;
        int cdy[] = { RADIUS/2, 0,RADIUS/2,SIDE,WIDTH,SIDE};
        CORNERS_DY = cdy;
    ...
    public int getNeighborI(int neighborIdx) {
        return mJ + NEIGHBORS_DJ[neighborIdx];
    }
    ...
    public int getNeighborJ(int neighborIdx) {
        return mI + NEIGHBORS_DI[mI % 2][neighborIdx];
    }
    ...
    public void setCellByPoint(int x, int y) {
        ...
        int tx = y - (ci % 2) * HEIGHT / 2;
        int cj = (int)Math.floor((float)tx/(float)HEIGHT);
